...
const App: React.FC = () => {
  ...
  return (
    <HashRouter>
      <ScrollToTop />
      <div className="min-h-screen flex flex-col text-gray-800 dark:text-gray-100 transition-colors duration-300">
        {/* Navigation Bar */}
        <header className="bg-[#2E7D32]/95 dark:bg-slate-900/95 backdrop-blur-md text-white shadow-lg sticky top-0 z-50 transition-colors">
          <div className="max-w-4xl mx-auto px-4 py-3 flex justify-between items-center">
            <Link to="/" className="flex items-center space-x-3 group">
              <div className="bg-white p-0.5 rounded-full shadow-sm transition-transform group-hover:scale-110 overflow-hidden w-10 h-10 md:w-14 md:h-14 flex items-center justify-center">
                <img 
                  src={logoUrl} 
                  alt="ShanimalDT Logo" 
              ...
              <h1 className="text-xl md:text-3xl font-bold tracking-tight">ShanimalDT</h1>
            </Link>
            <nav className="flex items-center space-x-4 md:space-x-6">
              <Link to="/" className="hover:text-green-200 text-xs md:text-sm font-bold uppercase tracking-wider transition-colors">হোম</Link>
              <Link to="/about" className="hover:text-green-200 text-xs md:text-sm font-bold uppercase tracking-wider transition-colors whitespace-nowrap">আমাদের সম্পর্কে</Link>
        ...
        {/* Footer */}
        <footer className="bg-[#5D4037] dark:bg-slate-950 text-white py-10 mt-auto shadow-[0_-10px_30px_rgba(0,0,0,0.05)] transition-colors">
